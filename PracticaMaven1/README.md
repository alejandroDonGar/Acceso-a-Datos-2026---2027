...
Después se quitó también Gson de `RepositorioTareas.java` (sigue declarado en el `pom.xml` porque las lecciones 6-15 piden añadirlo como dependencia de ejemplo, pero el proyecto final ya no llama a su API). `RepositorioTareas` lee y escribe el JSON a mano, con `String` y las clases de `java.nio.file`: al guardar construye el texto con `+` (concatenación normal, sin `StringBuilder`), y al cargar comprueba primero que el archivo tiene la forma esperada (empieza y termina con llave y contiene `"siguienteId"` y `"tareas"`) y luego separa cada línea con `split(":")`/`split(",")` y limpia el texto con `replace()` — nada de `append()`, `substring()` ni `indexOf()`. Si el archivo no tiene esa forma, se lanza `IOException` sin tocar el archivo, igual que antes.
